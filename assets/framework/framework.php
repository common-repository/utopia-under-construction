...
if (!class_exists('SeedProd_Framework')) {
	class SeedProd_Framework {
        ...
        function admin_enqueue_scripts( $hook_suffix ) {
			...
			wp_enqueue_script('jquery-ui-datepicker', plugins_url('datepicker.js',__FILE__), array('jquery', 'jquery-ui-core') );
			wp_enqueue_style('jquery.ui.theme', plugins_url('jquery-ui-custom.css',__FILE__));
			
			wp_enqueue_script('ki_script', plugins_url('ki_script.js',__FILE__), array('jquery', 'jquery-ui-core') );
			wp_register_script('utopia_admin_script',plugins_url('utopia_admin_script.js',__FILE__), array('jquery', 'jquery-ui-core'));
			$utopia_url = plugins_url('' , __FILE__ );
			$utopia_url = str_replace('framework','',$utopia_url);
			$params = array(
			  'utopia_url' =>  $utopia_url
			);
			wp_localize_script('utopia_admin_script', 'JSParams', $params );
			wp_enqueue_script('utopia_admin_script' );
        }
        ...
        function create_menu(){
            foreach ($this->menu as $v) {
                $this->pages[] = call_user_func_array($v['type'],array($v['page_name'],$v['menu_name'],$v['capability'],$v['menu_slug'],$v['callback']));
            }
                    ...
                    <div id="post-body">
                        <div id="post-body-content" >
                            <div class="meta-box-sortables ui-sortable">
								...
								<div class="donate-ki"><a href="https://www.krasotaiskusstva.com/promo/utopia/donate" target="_blank"><img src="https://www.krasotaiskusstva.com/promo/utopia/paypal.png" alt="" /></a></div>
                        	    </form>
                            </div>
                        </div>
                    </div>
					<div id="sidebar" class="ki-ad">
						
						<div class="ki"><a href="https://www.krasotaiskusstva.com/promo/utopia/go/" target="_blank"><img src="https://www.krasotaiskusstva.com/promo/utopia/ki.png" alt="" /></a></div>
					</div>
                </div>
        	</div>	
        	<?php
        ...
        function field_machine($args) {
        	...
        	switch($type){
        	    case 'textbox':
        	        ...
        	        break;
				 case 'datepicker':
        	        echo "<input id='$id' class='".(empty($class) ? 'mydatepicker ' : $class)."' name='{$setting_id}[$id]' type='text' value='".esc_attr(empty($options[$id]) ? $default_value : $options[$id])."' />
        	        <br><small class='description'>".(empty($desc) ? '' : $desc)."</small>";
        	        break;
                case 'image':
        	    ...
        	    case 'select':
				 ...
				 case 'select_image':
                    ...
                    break;
        	    case 'textarea':
                    echo "<textarea id='$id' class='large-text' name='{$setting_id}[$id]'>".(empty($options[$id]) ? $default_value : $options[$id])."</textarea>
        	        <br><small class='description'>".(empty($desc) ? '' : $desc)."</small>";
        	        break;
        	    case 'radio':
        	        foreach($option_values as $k=>$v){
